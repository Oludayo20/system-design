 ...
 *
 * Note: this file itself lives in `infrastructure/`, not `domain/` — the Data Access layer is
 * the only layer in this codebase allowed to know that TypeORM/Postgres exists at all.
 */
export const dataSourceOptions: DataSourceOptions = {
  type: 'postgres',
  host: process.env.POSTGRES_HOST ?? 'localhost',
  port: Number(process.env.POSTGRES_PORT ?? 5432),
  username: process.env.POSTGRES_USER ?? 'library',
  password: process.env.POSTGRES_PASSWORD ?? 'library_dev_password',
  database: process.env.POSTGRES_DB ?? 'library',
  entities: [__dirname + '/../../{books,members,loans}/infrastructure/*.orm-entity{.ts,.js}'],
